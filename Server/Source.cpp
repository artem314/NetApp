...
unsigned _stdcall Receive(void *lparamter)
{
	int count = 0;
	SOCKET client_sock = (SOCKET)lparamter;
	...
	for (;;)
	{
int err = recvn(client_sock, (char *)&FH, sizeof(FH));
...
int main(int argc, char* argv[])
{
	...
	if (SOCKET_ERROR != err)
	{
		for (;;)
		{
			...
			{
				HANDLE htread = (HANDLE)_beginthreadex(NULL, 0, Receive, (SOCKET *)(client_sock), 0, NULL);
				CloseHandle(htread);
			}
		}
	}
